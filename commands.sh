...
results="{
	...
	\"commitCountByDev\":\"$(git shortlog -s -n HEAD | head -5 | tr '\t' ' ' | awk '{printf "<tr><td>%s.&nbsp;</td><td>%-2s&nbsp;%2s&nbsp;</td><td>%d&nbsp;</td><td>(%.2f%s)</td></tr>", NR,$2,$3,$1,$1/'"$commitCount"'*100,"%"}')\",
	\"repoAge\":\"$(getRepoAge)\",
	\"fileCount\":\"$fileCount\",
	\"fileCountByType\":\"$(git ls-files | awk -F . '{print $NF}' | sort | uniq -c | sort -n -r  | head -5 | awk '{printf "<tr><td>%s.&nbsp;</td><td>%s&nbsp;</td><td>%d&nbsp;</td><td>(%.2f%s)</td></tr>", NR,$2,$1,$1/'"$fileCount"'*100,"%"}')\"
}"
